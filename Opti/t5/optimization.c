...
double get_step_backtracking(FuncInfo info, double* d, double *x, double c, double p, double alpha){
  double *g = newVector(n);
  info.gradient(x, n, g);
  double fd = dotproduct(g, d);
  double fx = info.function(x);
  double *x1 = newVector(n);
  while(1){
    memcpy(n, g, sizeof(double) * n); //copy direction to gradient vector
    scaleVector(g, n, alpha);
    sumVectors(x, g, g, n);
    double fxk = info.function(g);
...
double get_step(double alp, Step stp, FuncInfo info, double *gradient, double *x, int n){
  double step = alp;
  switch(stp){
    case StepFijo:
      break;
...
//steepest decent
double optimize_function(FuncInfo info, Step stp, double *x, int n, int iter, double tg, 
                          double tx, double tf, double step){
  double *x1  = newVector(n);
  ...
  for (int i = 0; i < iter; ++i)
  {
    ...
    if(vectorNorm(dir, n, 2) < tg) {
      printf("exit by tg\n");
      break;
    }
    step = get_step(step, stp, info, dir, x, n);
    ...
    printf("%i,\t%g\t st %g\n",i, info.function(x1, n), step); //printVector(x1, n);
